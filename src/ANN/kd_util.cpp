...
#define PA(i, d, dd) (pa[pidx[(i)] * (dd) + (d)])
				 ...
				 ANNorthRect &bnds) // bounding cube (returned)
{
	...
	{									 // find smallest enclosing rectangle
		ANNcoord lo_bnd = PA(0, d, dim); // lower bound on dimension d
		ANNcoord hi_bnd = PA(0, d, dim); // upper bound on dimension d
		for (int i = 0; i < n; i++)
		{
			if (PA(i, d, dim) < lo_bnd)
				lo_bnd = PA(i, d, dim);
			else if (PA(i, d, dim) > hi_bnd)
				hi_bnd = PA(i, d, dim);
		}
		bnds.lo[d] = lo_bnd;
		bnds.hi[d] = hi_bnd;
...
ANNcoord annSpread(   // compute point spread along dimension
	...
	int dd)			  // Dimension size
{
	ANNcoord min = PA(0, d, dd); // compute max and min coords
	ANNcoord max = PA(0, d, dd);
	for (int i = 1; i < n; i++)
	{
		ANNcoord c = PA(i, d, dd);
		if (c < min)
			min = c;
		else if (c > max)
			max = c;
...
{
	min = PA(0, d, dd); // compute max and min coords
	max = PA(0, d, dd);
	for (int i = 1; i < n; i++)
	{
		ANNcoord c = PA(i, d, dd);
	...
	int dim)		  // dimension of space
{
	...
	{ // compute spread along each dim
		ANNcoord spr = annSpread(pa, pidx, n, d, dim);
		if (spr > max_spr)
					...
					int n_lo)		  // split into n_lo and n-n_lo
{
	...
	while (l < r)
	{
		...
		ANNcoord c = PA(l, d, dd); // pivot value
		i = l;
		k = r;
		for (;;)
		{ // pivot about c
	...
	{							   // search for next smaller item
		ANNcoord c = PA(0, d, dd); // candidate for max
	...
	cv = (PA(n_lo - 1, d, dd) + PA(n_lo, d, dd)) / 2.0;
}
	...
	int &br2)		  // second break (values == cv)
{
	...
	{ // partition pa[0..n-1] about cv
		while (l < n && PA(l, d, dd) < cv)
			l++;
		while (r >= 0 && PA(r, d, dd) >= cv)
			r--;
		if (l > r)
			break;
		PASWAP(l, r);
