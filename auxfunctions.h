...
// the exponent of a quaternion: from (0, theta*vec) (axis angle) to (cos(theta), sin(theta)*vec))
...
// returning a rotation matrix R for quaternion q so that R*vec.transpose() = QRot(vec,
// q).transpose() for a vec:1x3 (RowVector3d)
