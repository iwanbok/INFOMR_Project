...
inline void compProjectionIntegrals(const Eigen::MatrixXd &V, const Eigen::RowVectorXi &f)
{
	double a0, a1, da;
	double b0, b1, db;
...
inline void compFaceIntegrals(const Eigen::MatrixXd &V, const Eigen::RowVectorXi &f,
...
inline void compVolumeIntegrals(const Eigen::MatrixXd &V, const Eigen::MatrixXi &T)
{
	Eigen::MatrixXd N;
...
// computes mass (by uniform density), center of mass, and inertia tensor (around the COM with the
// canonical axis system) for a polygon

inline void getCOMandInvIT(const Eigen::MatrixXd &V, const Eigen::MatrixXi &T, const double density,
